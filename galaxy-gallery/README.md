...
| Element | What it is |
|---|---|
| **Intro card** | Glass panel, `Galaxy Gallery` title, animated sticker, `Start` button. **Skipped by default** — the scene auto-starts, so the card is never shown (see "auto-start" in `main.js`) |
...
| **Accretion disc** | 11,000 golden particles orbiting a black-hole sphere, plus warm halo sprite |
| **Beam** | 5,600 magenta particles streaming upward from the core, recycling in a loop |
| **Heart** | 13,000 particles on a parametric heart, spaced by **arc length** so they never clump at the tip or lobes — slim rim band + light inner veil, billboarded, with a heartbeat pulse. Plus a 3,200-particle aura, 260 orbiting sparks and 16 drifting embers |
...
```js
phrases: ['MY HEART IS YOURS', 'ETERNAL LOVE ✦', ...]
...
heart:   { count, scale, height, color, rimColor,
           auraCount, sparkCount, emberCount }
```
